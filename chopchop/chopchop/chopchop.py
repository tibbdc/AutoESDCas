...
# CONFIG
# f_p = sys.path[0]
# config_path = f_p + "/data/input/config_local.json" if os.path.isfile(f_p + "/data/input/config_local.json") else f_p + "/data/input/config.json"
...
class Cas9(Guide):
    def __init__(self, *args, **kwargs):
        ...
        if self.scoringMethod == "ALKAN_2018" or self.scoringMethod == "ALL":
            ...
            self.CoefficientsScore["ALKAN_2018"] = CRISPRoff_score(self.strandedGuideSeq)
...
def parseBowtie(guideClass, bowtieResultsFile, checkMismatch, scoreGC, scoreSelfComp,
    ...
    maxOffTargets = int(maxOffTargets)
    curr_guide = None
    guide_list = []
    ...
    if mode: # Cas9, Cpf1, Nickase and not TALEN
        sam[14] = sam[0].str[-(len(PAM) + 1):]   #PAM
...
def sort_CRISPR_guides(guides):
    ...
    for i in guides:
        pam_len = len(i.PAM)
        i.strandedGuideSeq = i.strandedGuideSeq[:-pam_len]
        aa.append(i)
    
    return sorted(aa, key=attrgetter('score'))
...
def main():
    ...
    parser.add_argument("-config", "--config", help='input params file', required=True) 
    ...
    if ISOFORMS:
        ...
        if args.MODE == CRISPR:
            ...
            if args.scoringMethod == "ALL":
                columns = common_header + "\tXU_2015\tDOENCH_2014\tDOENCH_2016\tMORENO_MATEOS_2015\tCHARI_2015\tG_20\tALKAN_2018\tZHANG_2019"
                print(common_header + "\tXU_2015\tDOENCH_2014\tDOENCH_2016\tMORENO_MATEOS_2015\tCHARI_2015\tG_20\tALKAN_2018\tZHANG_2019")
            else:
                columns = common_header + "\tEfficiency"
            ...
            temp_value = []
            for i in range(len(sortedOutput)):
                temp = "%s\t%s" % (i+1, sortedOutput[i])
                temp = temp.split('\t')
                print ("%s\t%s" % (i+1, sortedOutput[i]))
                temp_value.append(temp)
                resultCoords.append([sortedOutput[i].start, sortedOutput[i].score, sortedOutput[i].guideSize, sortedOutput[i].strand])
            output_df = pd.DataFrame(columns = columns,data=temp_value)
            # if output_df.
            output_df.to_csv(args.outputDir+'result.csv',index=False)   
